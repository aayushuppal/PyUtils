class BinaryHeapPriorityQueue(object):
    ...
    def __sink(self, n):
        ...
        while 2*k <= self._currentSize:
            j = 2*k
            if j < self._currentSize\
            and self._heapList[j+1] == self._prefer(self._heapList[j], self._heapList[j+1]):
                j += 1
    ...
    def __swim(self, n):
        k = n
        while (k > 1)\
        and self._heapList[k] == self._prefer(self._heapList[k], self._heapList[int(k/2)]):
